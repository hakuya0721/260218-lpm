    ...
    def __init__(
        ...
        if vqgan:
            critic_segm_params += list(
                        filter(
                            lambda p: p.requires_grad,
                            actor_critic.net.state_encoder.mask_embed_vq.parameters(),
                        )
                    )
            critic_segm_params += list(
                        filter(
                            lambda p: p.requires_grad,
                            actor_critic.net.state_encoder.mask_embed_decoder.parameters(),
                        )
                    )
            critic_segm_params += list(
                        filter(
                            lambda p: p.requires_grad,
                            actor_critic.net.state_encoder.mask_embed_discriminator.parameters(),
                        )
                    )
            critic_segm_params += list(
                        filter(
                            lambda p: p.requires_grad,
                            actor_critic.net.state_encoder.mask_embed_quant_conv.parameters(),
                        )
                    )
            critic_segm_params += list(
                        filter(
                            lambda p: p.requires_grad,
                            actor_critic.net.state_encoder.mask_embed_post_quant_conv.parameters(),
                        )
                    )
        else:
             critic_segm_params += list(
                          filter(
                             lambda p: p.requires_grad,
                             actor_critic.net.state_encoder.mask_embed.parameters(),
                         )
                     )

        params = [
            {
                "params": critic_segm_params,
                "lr": lr,
                "eps": eps,
            },
            {
    ...
    def update(
        ...
        for epoch in range(self.ppo_epoch):
            ...
            for _bid, batch in enumerate(data_generator):
                ...
                print("Value loss:", torch.mean(value_loss))
