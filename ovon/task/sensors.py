...
from habitat.core.simulator import RGBSensor, SemanticSensor, Sensor, SensorTypes, Simulator
...
class MultiSemanticSensor(Sensor):
    cls_uuid: str = "multi_semantic_sensor"
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        semantic_sensor_uuids = [
            uuid for uuid, sensor in sensors.items() if isinstance(sensor, SemanticSensor)
        ]
        if len(semantic_sensor_uuids) != 1:
            raise ValueError(
                "MultiSemanticSensor requires one semantic sensor,"
    ...
    def _get_observation_space(self, *args: Any, **kwargs: Any):
        return self._sim.sensor_suite.observation_spaces.spaces[self._semantic_sensor_uuid]
