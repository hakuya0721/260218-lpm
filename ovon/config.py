...
##########################################################################
@dataclass
class MultiSemanticSensorConfig(LabSensorConfig):
    type: str = "MultiSemanticSensor"
...
@dataclass
class SegmentationUpdatesConfig(HabitatBaselinesBaseConfig):
    enabled: bool = True
    frequency: int = 10
...
class OVONPolicyConfig(PolicyConfig):
    ...
    segm_update_config: SegmentationUpdatesConfig = field(
        default_factory=lambda: SegmentationUpdatesConfig()
    )
...
cs.store(
    package="habitat.task.lab_sensors.multi_semantic_sensor",
    group="habitat/task/lab_sensors",
    name="multi_semantic_sensor",
    node=MultiSemanticSensorConfig,
)
