...
from habitat_baselines.rl.ppo import SegmentationAuxiliary
...
class PointNavResNetCLIPPolicy(NetPolicy):
    def __init__(
        ...
        use_odom: bool = False,
        segm_mask_input: bool = False,
        segm_loss_enabled: bool = False,
        segmentation_source: str = "gt",
        **kwargs,
    ):
        ...
        super().__init__(
            OVONNet(
                ...
                use_odom=use_odom,
                segm_mask_input=segm_mask_input,
                segm_loss_enabled=segm_loss_enabled,
    ...
    def from_config(
        ...
        **kwargs,
        ...
        return cls(
            ...
            segm_mask_input=config.habitat_baselines.rl.policy.segm_mask_input,
            segm_loss_enabled=config.habitat_baselines.rl.policy.segm_loss_enabled,
            segmentation_source=config.habitat_baselines.rl.policy.segmentation_source,
            **kwargs,
        )
    ...
    def freeze_state_encoder(self):
        ...
        if vqgan:
            blacklisted_layers = [
                ...
                "mask_embed_vq",
                "mask_embed_decoder",
                "mask_embed_discriminator",
                "mask_embed_quant_conv",
                "mask_embed_post_quant_conv"
            ]
        else:
            blacklisted_layers = [
                "visual_encoder",
    ...
    def unfreeze_new_params(self):
        ...
        for param in state_encoder_params:
            param.requires_grad_(True)
        print("unfnew", whitelisted_names)
...
class OVONNet(Net):
    def __init__(
        ...
        **kwargs,
        ...
        self.segm_mask_input = segm_mask_input
        self.segm_loss_enabled = segm_loss_enabled
        ...
        if self.segm_mask_input:
            rnn_input_size_info["segm_mask_input"] = 128
        ...
        self.obj_name_to_ids = {i:ii for ii,i in enumerate(mp3d_names)}
        self.ids_to_obj_name = {ii:i for ii,i in enumerate(mp3d_names)}
        self.cache = load_pickle("data/text_embeddings/siglip.pkl")

        self.segm_loss = SegmentationAuxiliary(action_space, self)
        ...
        if self.segm_mask_input:
            ...
            self.semantic_encoder = ResNetEncoder(
                    spaces.Dict({"depth": observation_space.spaces["depth"]}),
                    baseplanes=32,
                    ngroups=32 // 2,
                    make_backbone=getattr(resnet, "resnet9"),
            )
        
        if self.segmentation_source == "yolo_val_unseen":
            with open("cats_val_unseen.json", "r") as f:
                data = json.load(f)
    ...
    def forward(
        ...
        if self.segm_mask_input:
            ...
            if self.segmentation_source == "gt":
                segmentation_mask = observations["semantic"].float()
            else:

                if "segm_model" in observations:
                    segmentation_mask = observations["segm_model"].float()
                else:
                    segmentation_mask = torch.zeros_like(observations["semantic"].float())
                    results = self.segm_model[0].predict(
                        observations['rgb'].permute(0, 3, 1, 2) / 255,
                        save=False,
                        imgsz=224,
                        device="cuda",
                        conf=0.01
                    ...
                    for i, r in enumerate(results):
                        if r.masks is not None:
                            target = object_goal[i]
                            dists = torch.sum(torch.abs(self.cache_values - target), dim=1)
                            min_idx = torch.argmin(dists)

                            target_class = min_idx.item()
                            masks = r.masks.data

                            classes = r.boxes.cls.long()
                            confs = r.boxes.conf
                            if self.cats_text[target_class] in self.conf_per_cat:
                                selected_indices = ((classes == target_class) & (confs > self.conf_per_cat[self.cats_text[target_class]])).nonzero(as_tuple=True)[0]
                            else:
                                print("Not unseen:", self.cats_text[target_class])
                                selected_indices = (classes == target_class).nonzero(as_tuple=True)[0]
                            
                            selected_masks = masks[selected_indices]
                            
                            if selected_masks.shape[0] > 0:
                                segmentation_mask[i, :, :, 0] = selected_masks.any(dim=0).squeeze().detach()


            new_segm_mask = self.semantic_encoder({"depth": segmentation_mask.float()})
            new_segm_mask = F.avg_pool2d(new_segm_mask, 4).squeeze(3).squeeze(2)
            # print(new_segm_mask.shape)
            x.append(new_segm_mask)

            
        out = torch.cat(x, dim=1)

        out, rnn_hidden_states, (vq_loss, orthogonal_loss, reconstructed_image) = self.state_encoder(
        ...
        #if self.segm_mask_input:
        #    probabilities = reconstructed_image.sigmoid()
        #    probabilities = F.interpolate(probabilities, size=(224, 224)).permute(0, 2, 3, 1)
        #    new_segm_mask = self.prev_semantic_encoder({"depth": probabilities})
        #    new_segm_mask = F.avg_pool2d(new_segm_mask, 4).squeeze(3).squeeze(2)
        ...
        if self.segm_mask_input:
            ...
            aux_loss_state = {
                ...
                "orthogonal_loss": orthogonal_loss,
                'pos_weight': 1.0,
                'prev_segm_masks': torch.zeros((prev_actions.shape[0], 128), device="cuda")
            }
    ...
    def segment(
        ...
        cats_set = torch.unique(observations['multi_semantic_sensor'])

        # and self.ids_to_obj_name[int(x)] in self.cats_train

        str_set = {
            self.ids_to_obj_name[int(x)]
            for x in cats_set.tolist()
            if self.ids_to_obj_name[int(x)] in self.cache and self.ids_to_obj_name[int(x)] in self.cats_train and torch.sum(observations['multi_semantic_sensor'] == int(x)) / observations['multi_semantic_sensor'].shape[0] > 10
        }        

        category = random.choice(list(str_set))
        print(category)

        object_goal = torch.from_numpy(self.cache[category]).cuda().repeat(visual_feats.shape[0], 1)
        ...
        binary_gt_sem = torch.zeros_like(observations['multi_semantic_sensor'])
        
        category_idx = self.obj_name_to_ids[category]
        binary_gt_sem[observations['multi_semantic_sensor'] == category_idx] = 1.0

        """ Previous masks
        # Create an empty tensor of shape [1, C, H, W] (same shape as one sample)
        empty_tensor = torch.zeros((1, binary_gt_sem.shape[1], binary_gt_sem.shape[2], binary_gt_sem.shape[3]), dtype=binary_gt_sem.dtype, device=binary_gt_sem.device)

        
        # Concatenate the empty tensor at the beginning
        previous_segm = torch.cat([empty_tensor, binary_gt_sem[:-1]], dim=0)
        #previous_segm = torch.zeros_like(previous_segm)

        if self.segm_mask_input:
            prev_actions = prev_actions.squeeze(-1)
            start_token = torch.zeros((prev_actions.shape[0], 128), device="cuda")
            prev_segm_masks = self.prev_semantic_encoder({"depth": previous_segm.float()})
            prev_segm_masks = F.avg_pool2d(prev_segm_masks, 4).squeeze(3).squeeze(2)
            filtered_prev_segm_masks = torch.clone(prev_segm_masks)
            filtered_prev_segm_masks[~masks[:, -1:].view(-1)] = 0
            
            x.append(filtered_prev_segm_masks)
        """

        if self.segm_mask_input:
            if self.segmentation_source == "gt":
                segmentation_mask = binary_gt_sem.float()
            else:
                segmentation_mask = torch.zeros_like(observations["semantic"].float())

                # print(observations['rgb'].shape, segmentation_mask.shape)
                ...
                for i, r in enumerate(results):
                    if r.masks is not None:
                        object_goal_text = category
                        target_class = self.cats_text.index(object_goal_text)
                        masks = r.masks.data
                        #print(masks.shape)
                        classes = r.boxes.cls.long()
                        selected_indices = (classes == target_class).nonzero(as_tuple=True)[0]
            ...
            segm_masks = self.semantic_encoder({"depth": binary_gt_sem.float()})
            segm_masks = F.avg_pool2d(segm_masks, 4).squeeze(3).squeeze(2)         
            x.append(segm_masks)

        out = torch.cat(x, dim=1)

        print("rnn_build_seq_info", rnn_build_seq_info.keys())
        ...
        print("Mask sum:", torch.sum(binary_gt_sem))
        segmentation_loss = self.segm_loss(aux_loss_state, gt_segmentation)
        print(segmentation_loss)
        return segmentation_loss

        
    def adversarial_loss(self,pred_fake, pred_real):
