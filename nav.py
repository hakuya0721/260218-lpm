...
class Collisions(Measure):
    ...
    def reset_metric(self, episode, *args: Any, **kwargs: Any):
        self._metric = {"count": 0, "is_collision": False, "total_steps": 0}

    def update_metric(self, episode, action, *args: Any, **kwargs: Any):
        self._metric["is_collision"] = False
        self._metric["total_steps"] += 1
        if self._sim.previous_step_collided:
            self._metric["count"] += 1
            self._metric["is_collision"] = True

    def get_metric(self):
        # Compute proportion of steps that were collisions
        if self._metric["total_steps"] > 0:
            self._metric["proportion"] = self._metric["count"] / self._metric["total_steps"]
        else:
            self._metric["proportion"] = 0.0
        return self._metric
