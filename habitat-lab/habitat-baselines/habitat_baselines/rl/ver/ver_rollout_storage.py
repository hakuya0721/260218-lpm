...
class VERRolloutStorage(RolloutStorage):
    ...
    next_prev_segm_masks: torch.Tensor
    ...
    def __init__(
        ...
        assert isinstance(self.buffers["prev_segm_masks"], torch.Tensor)
        self._aux_buffers["next_prev_actions"] = self.buffers["prev_actions"][
            0
        ].clone()
        self._aux_buffers["next_prev_segm_masks"] = self.buffers["prev_segm_masks"][
            0
        ].clone()
