    ...
    def _eval_checkpoint(
        ...
        while (
            ...
            with inference_mode():
                ...
                prev_segm_masks.copy_(segm_masks)
            ...
            for i in range(n_envs):
                ...
                if len(self.config.habitat_baselines.eval.video_option) > 0:
                    ...
                    probabilities = aux_output['reconstructed_image'].sigmoid()

                    probabilities = F.interpolate(probabilities, size=(224, 224))

                    # Convert to NumPy for visualization
                    mask_np = probabilities.squeeze(0).unsqueeze(3).cpu().numpy()

                    batch['object_goal_mask'] = mask_np
                ...
                if not not_done_masks[i].item():
                    ...
                    for stat_key in next(iter(stats_episodes.values())).keys():
                        print(stat_key, np.mean(
                            [v[stat_key] for v in stats_episodes.values()]
                        ))
                    #print(infos[i])
                    if infos[i]['object_goal'] not in self.sr_per_cat:
                        self.sr_per_cat[infos[i]['object_goal']] = [infos[i]['success']]
                    else:
                        self.sr_per_cat[infos[i]['object_goal']].append(infos[i]['success'])
                
                    stats = {}
                    for k, v in self.sr_per_cat.items():
                        stats[k] = np.mean(v)
