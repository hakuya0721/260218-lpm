...
class InferenceWorkerProcess(ProcessBase):
    ...
    def step(self) -> Tuple[bool, List[Tuple[int, int]]]:
        ...
        with self.timer.avg_time("step policy"):
            ...
            prev_segm_masks = self.rollouts.next_prev_segm_masks[environment_ids]
            if self._static_encoder:
                obs[
                    PointNavResNetNet.PRETRAINED_VISUAL_FEATURES_KEY
                ] = self.visual_encoder(obs)

                if self.segm_model is not None and random.random() < self.update / 5000:
                    #print("P predicted segmentation:", self.num_updates / 1000)
                    object_goal = obs[ClipObjectGoalSensor.cls_uuid]

                    segmentation_mask = torch.zeros_like(obs["semantic"].float())
                    #start = torch.cuda.Event(enable_timing=True)
                    #end = torch.cuda.Event(enable_timing=True)

                    #start.record()
                    # 1. Run batched prediction
                    results = self.segm_model.predict(
                        obs['rgb'].permute(0, 3, 1, 2) / 255.0,
                        save=False,
                        imgsz=224,
                        device="cuda",
                        half=True
                    ...
                    #    - Only keep indices where masks exist
                    has_masks = torch.tensor([r.masks is not None for r in results], device="cuda")

                    # 3. Stack all targets: [B, D]
                    targets = object_goal  # [B, D]

                    # 4. Compute L1 distances between targets and cache: [B, K]
                    dists = torch.cdist(targets, self.cache_values, p=1)

                    # 5. Get best-matching class for each item: [B]
                    min_indices = torch.argmin(dists, dim=1)

                    # 6. Process results in a batched, filtered way
                    # Preallocate and fill segmentation mask
                    for b_idx in has_masks.nonzero(as_tuple=True)[0]:
                        r = results[b_idx]
            ...
            if not final_batch:
                ...
                self.rollouts.next_prev_segm_masks.index_copy_(
                    0, environment_ids, prev_segm_masks
                )
        ...
        with self.timer.avg_time("update storage"):
            current_step = TensorDict.from_tree(
                dict(
                    ...
                    prev_segm_masks=prev_segm_masks,
