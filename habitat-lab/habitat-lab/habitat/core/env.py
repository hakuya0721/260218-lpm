...
class Env:
    ...
    def reset(self) -> Observations:
        ...
        self.object_goal = self._current_episode.object_category
        ...
        for k,v in self.instance_id_to_label_id.items():
            try:
                if self.index_to_title_map[self.instance_id_to_label_id[k]] == self.object_goal:
                    self.sem_to_model[observations['semantic']== k] = 1.0
            except:
                if any([x == self.object_goal for x in self.index_to_title_map[self.instance_id_to_label_id[k]]]):
                    self.sem_to_model[observations['semantic']== k] = 1.0

        instance_to_label = np.frompyfunc(self.instance_id_to_label_id.get, 1, 1)
        all_cats = instance_to_label(observations['semantic']).astype(object)
        
        index_to_title = np.frompyfunc(self.index_to_title_map.get, 1, 1)
        all_cats = index_to_title(all_cats)
        
        obj_name_to_ids = np.frompyfunc(self.obj_name_to_ids.get, 1, 1)
        all_cats = obj_name_to_ids(all_cats)

        observations['semantic'] = self.sem_to_model
        try:
